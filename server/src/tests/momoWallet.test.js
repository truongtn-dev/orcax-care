...
describe("UC-19.3 Momo Wallet Payment", () => {
  let server;
  let baseUrl;
  let patientUser;
  ...
  beforeEach(async () => {
    ...
    patientUser = await User.create({
      email: "patient.momo@orcaxcare.com",
      passwordHash: "hash",
      role: "patient",
      fullName: "Momo Patient",
      isActive: true,
  ...
  test("lists Momo as enabled payment method", async () => {
    const res = await fetch(`${baseUrl}/api/patient/wallet`, {
      headers: { Authorization: await authHeaderFor(patientUser) },
    });
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.ok(body.paymentMethods.some((item) => item.id === "momo"));
    assert.equal(body.momoMockMode, true);
  });

  test("completes mock Momo top-up with reference id", async () => {
    const createRes = await fetch(`${baseUrl}/api/patient/wallet/topups/momo`, {
      method: "POST",
      headers: {
    ...
    const confirmRes = await fetch(`${baseUrl}/api/patient/wallet/momo/mock-confirm`, {
      method: "POST",
      headers: {
        Authorization: await authHeaderFor(patientUser),
        "Content-Type": "application/json",
    ...
    assert.equal(receiptBody.receipt.provider, "momo");
  ...
  test("does not change balance when mock confirm uses wrong order", async () => {
    ...
    const confirmRes = await fetch(`${baseUrl}/api/patient/wallet/momo/mock-confirm`, {
      ...
      body: JSON.stringify({ orderId: "MOMO-INVALID" }),
    });

...
describe("UC-8.1.1 Reschedule Appointment", () => {
  ...
  test("lists only authenticated patient appointments", async () => {
    ...
    assert.equal(body.items[0].referenceCode, "APT-RS-001");
    assert.equal(body.items[0].patientName, "Reschedule Patient");
    assert.equal(body.items[0].doctorName, "Dr. Reschedule");
  });
